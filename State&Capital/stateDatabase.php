<?php
$states = [
    [
        "state" => "Abia Umuahia",
        "governor" => "Alex Otti",
        "population" => "3.7 million",
        "mineral_resources" => ["Lead", "Zinc", "Limestone"],
        "slogan" => "God’s Own State",
        "geo_zone" => "South East",
        "year_created" => 1991
    ],
    [
        "state" => "Adamawa Yola",
        "governor" => "Ahmadu Fintiri",
        "population" => "4.2 million",
        "mineral_resources" => ["Gypsum", "Kaolin", "Bentonite"],
        "slogan" => "Land of Beauty",
        "geo_zone" => "North East",
        "year_created" => 1991
    ],

    [
        "state" => "Akwa Ibom Uyo",
        "governor" => "Umo Eno",
        "population" => "5.5 million",
        "mineral_resources" => ["Petroleum", "Natural Gas", "Salt"],
        "slogan" => "Land of Promise",
        "geo_zone" => "South South",
        "year_created" => 1987
    ],
    [
        "state" => "Anambra Awka",
        "governor" => "Charles Soludo",
        "population" => "4.5 million",
        "mineral_resources" => ["Lignite", "Clay", "Ironstone"],
        "slogan" => "Light of the Nation",
        "geo_zone" => "South East",
        "year_created" => 1976
    ],
    [
        "state" => "Bauchi Bauchi",
        "governor" => "Bala Mohammed",
        "population" => "6.5 million",
        "mineral_resources" => ["Coal", "Limestone", "Gypsum"],
        "slogan" => "Pearl of Tourism",
        "geo_zone" => "North East",
        "year_created" => 1976
    ],
    [
        "state" => "Bayelsa Yenagoa",
        "governor" => "Douye Diri",
        "population" => "2.3 million",
        "mineral_resources" => ["Crude Oil", "Natural Gas"],
        "slogan" => "Glory of All Lands",
        "geo_zone" => "South South",
        "year_created" => 1996
    ],
    [
        "state" => "Benue Makurdi",
        "governor" => "Hyacinth Alia",
        "population" => "5.7 million",
        "mineral_resources" => ["Limestone", "Gypsum", "Barytes"],
        "slogan" => "Food Basket of the Nation",
        "geo_zone" => "North Central",
        "year_created" => 1976
    ],
    [
        "state" => "Borno Maiduguri",
        "governor" => "Babagana Zulum",
        "population" => "5.9 million",
        "mineral_resources" => ["Lignite", "Gypsum", "Clay"],
        "slogan" => "Home of Peace",
        "geo_zone" => "North East",
        "year_created" => 1976
    ],
    [
        "state" => "Cross River Calabar",
        "governor" => "Bassey Otu",
        "population" => "3.7 million",
        "mineral_resources" => ["Salt", "Limestone", "Clay"],
        "slogan" => "The People's Paradise",
        "geo_zone" => "South South",
        "year_created" => 1967
    ],
    [
        "state" => "Delta Asaba",
        "governor" => "Sheriff Oborevwori",
        "population" => "5.6 million",
        "mineral_resources" => ["Crude Oil", "Natural Gas", "Lignite"],
        "slogan" => "The Big Heart",
        "geo_zone" => "South South",
        "year_created" => 1991
    ],
    [
        "state" => "Ebonyi Abakaliki",
        "governor" => "Francis Nwifuru",
        "population" => "3.0 million",
        "mineral_resources" => ["Lead", "Salt", "Limestone"],
        "slogan" => "Salt of the Nation",
        "geo_zone" => "South East",
        "year_created" => 1996
    ],
    [
        "state" => "Edo Benin City",
        "governor" => "Godwin Obaseki",
        "population" => "5.0 million",
        "mineral_resources" => ["Crude Oil", "Limestone", "Clay"],
        "slogan" => "Heartbeat of the Nation",
        "geo_zone" => "South South",
        "year_created" => 1991
    ],
    [
        "state" => "Ekiti Ado-Ekiti",
        "governor" => "Biodun Oyebanji",
        "population" => "2.7 million",
        "mineral_resources" => ["Granite", "Kaolin", "Feldspar"],
        "slogan" => "Fountain of Knowledge",
        "geo_zone" => "South West",
        "year_created" => 1996
    ],
    [
        "state" => "Enugu Enugu",
        "governor" => "Peter Mbah",
        "population" => "4.4 million",
        "mineral_resources" => ["Coal", "Limestone", "Clay"],
        "slogan" => "Coal City State",
        "geo_zone" => "South East",
        "year_created" => 1991
    ],
    [
        "state" => "Gombe Gombe",
        "governor" => "Muhammadu Inuwa Yahaya",
        "population" => "3.3 million",
        "mineral_resources" => ["Gypsum", "Coal", "Limestone"],
        "slogan" => "Jewel of the Savannah",
        "geo_zone" => "North East",
        "year_created" => 1996
    ],
    [
        "state" => "Imo Owerri",
        "governor" => "Hope Uzodinma",
        "population" => "5.4 million",
        "mineral_resources" => ["Lead", "Zinc", "Natural Gas"],
        "slogan" => "Eastern Heartland",
        "geo_zone" => "South East",
        "year_created" => 1976
    ],
    [
        "state" => "Jigawa Dutse",
        "governor" => "Umar Namadi",
        "population" => "6.0 million",
        "mineral_resources" => ["Granite", "Kaolin"],
        "slogan" => "The New World",
        "geo_zone" => "North West",
        "year_created" => 1991
    ],
    [
        "state" => "Kaduna Kaduna",
        "governor" => "Uba Sani",
        "population" => "8.2 million",
        "mineral_resources" => ["Gold", "Tin", "Gemstone"],
        "slogan" => "Centre of Learning",
        "geo_zone" => "North West",
        "year_created" => 1967
    ],
    [
        "state" => "Kano Kano",
        "governor" => "Abba Kabir Yusuf",
        "population" => "13.4 million",
        "mineral_resources" => ["Tin", "Gypsum", "Lead"],
        "slogan" => "Centre of Commerce",
        "geo_zone" => "North West",
        "year_created" => 1967
    ],
    [
        "state" => "Katsina Katsina",
        "governor" => "Dikko Radda",
        "population" => "9.1 million",
        "mineral_resources" => ["Kaolin", "Asbestos"],
        "slogan" => "Home of Hospitality",
        "geo_zone" => "North West",
        "year_created" => 1987
    ],
    [
        "state" => "Kebbi Birnin Kebbi",
        "governor" => "Nasir Idris",
        "population" => "4.4 million",
        "mineral_resources" => ["Gold", "Clay"],
        "slogan" => "Land of Equity",
        "geo_zone" => "North West",
        "year_created" => 1991
    ],
    [
        "state" => "Kogi Lokoja",
        "governor" => "Ahmed Usman Ododo",
        "population" => "4.5 million",
        "mineral_resources" => ["Iron Ore", "Coal", "Limestone"],
        "slogan" => "The Confluence State",
        "geo_zone" => "North Central",
        "year_created" => 1991
    ],
    [
        "state" => "Kwara Ilorin",
        "governor" => "AbdulRahman AbdulRazaq",
        "population" => "3.1 million",
        "mineral_resources" => ["Limestone", "Marble"],
        "slogan" => "State of Harmony",
        "geo_zone" => "North Central",
        "year_created" => 1967
    ],
    [
        "state" => "Lagos Ikeja",
        "governor" => "Babajide Sanwo-Olu",
        "population" => "20.5 million",
        "mineral_resources" => ["Bitumen", "Clay"],
        "slogan" => "Centre of Excellence",
        "geo_zone" => "South West",
        "year_created" => 1967
    ],
    [
        "state" => "Nasarawa Lafia",
        "governor" => "Abdullahi Sule",
        "population" => "2.5 million",
        "mineral_resources" => ["Salt", "Tin"],
        "slogan" => "Home of Solid Minerals",
        "geo_zone" => "North Central",
        "year_created" => 1996
    ],
    [
        "state" => "Niger Minna",
        "governor" => "Mohammed Umar Bago",
        "population" => "5.1 million",
        "mineral_resources" => ["Gold", "Talc"],
        "slogan" => "The Power State",
        "geo_zone" => "North Central",
        "year_created" => 1976
    ],
    [
        "state" => "Ogun Abeokuta",
        "governor" => "Dapo Abiodun",
        "population" => "6.2 million",
        "mineral_resources" => ["Granite", "Limestone"],
        "slogan" => "Gateway State",
        "geo_zone" => "South West",
        "year_created" => 1976
    ],
    [
        "state" => "Ondo Akure",
        "governor" => "Lucky Aiyedatiwa",
        "population" => "4.8 million",
        "mineral_resources" => ["Bitumen", "Limestone"],
        "slogan" => "Sunshine State",
        "geo_zone" => "South West",
        "year_created" => 1976
    ],
    [
        "state" => "Osun Osogbo",
        "governor" => "Ademola Adeleke",
        "population" => "5.1 million",
        "mineral_resources" => ["Gold", "Granite"],
        "slogan" => "State of the Living Spring",
        "geo_zone" => "South West",
        "year_created" => 1991
    ],
    [
        "state" => "Oyo Ibadan",
        "governor" => "Seyi Makinde",
        "population" => "7.8 million",
        "mineral_resources" => ["Granite", "Marble"],
        "slogan" => "Pace Setter State",
        "geo_zone" => "South West",
        "year_created" => 1976
    ],
    [
        "state" => "Plateau Jos",
        "governor" => "Caleb Mutfwang",
        "population" => "4.2 million",
        "mineral_resources" => ["Tin", "Columbite"],
        "slogan" => "Home of Peace and Tourism",
        "geo_zone" => "North Central",
        "year_created" => 1976
    ],
    [
        "state" => "Rivers Port Harcourt",
        "governor" => "Siminalayi Fubara",
        "population" => "7.3 million",
        "mineral_resources" => ["Crude Oil", "Natural Gas"],
        "slogan" => "Treasure Base of the Nation",
        "geo_zone" => "South South",
        "year_created" => 1967
    ],
    [
        "state" => "Sokoto Sokoto",
        "governor" => "Ahmed Aliyu",
        "population" => "5.1 million",
        "mineral_resources" => ["Limestone", "Gypsum"],
        "slogan" => "Seat of the Caliphate",
        "geo_zone" => "North West",
        "year_created" => 1976
    ],
    [
        "state" => "Taraba Jalingo",
        "governor" => "Agbu Kefas",
        "population" => "3.3 million",
        "mineral_resources" => ["Kaolin", "Barytes"],
        "slogan" => "Nature’s Gift to the Nation",
        "geo_zone" => "North East",
        "year_created" => 1991
    ],
    [
        "state" => "Yobe Damaturu",
        "governor" => "Mai Mala Buni",
        "population" => "3.4 million",
        "mineral_resources" => ["Lignite", "Gypsum"],
        "slogan" => "Pride of the Sahel",
        "geo_zone" => "North East",
        "year_created" => 1991
    ],
    [
        "state" => "Zamfara Gusau",
        "governor" => "Dauda Lawal",
        "population" => "4.5 million",
        "mineral_resources" => ["Gold", "Copper"],
        "slogan" => "Farming is Our Pride",
        "geo_zone" => "North West",
        "year_created" => 1996
    ],
    [
        "state" => "FCT Abuja",
        "governor" => "Nyesom Wike (Minister)",
        "population" => "3.6 million",
        "mineral_resources" => ["Granite", "Marble"],
        "slogan" => "Centre of Unity",
        "geo_zone" => "Central",
        "year_created" => 1991
    ]
];
